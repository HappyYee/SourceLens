...
- Current status: UX batch U3 (settings overhaul + sidebar drag-move) implemented on the branch. No schema changes. UX batches U1/U2/U3 complete pending this verification; U4 small pool remains optional.
...
- Tests run (sandbox): `npm test` 197/197 (+3: type-key 1, tree-move 2); `npm run build` passed.
- Known failures: none in sandbox.
- Next recommended task: Codex verifies U3 checklist and merges. Afterwards: usage observation resumes; U4 small pool (date-format unification, folder aggregation view, global read-all, export双轨合一) on demand.
- Summary: U3 — structure management becomes calm (read-only rows, edit on demand) and direct (drag the sidebar).
