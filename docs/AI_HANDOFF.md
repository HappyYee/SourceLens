...
- Current status: Phase 3a (first archive-schema migration) is implemented and rehearsed in the sandbox; awaiting Codex local backup + migration + verification, then merge to `main`.
...
- Tests run (sandbox): `npm test` 178/178; `npm run build` passed. Pending Codex: real-DB backup → `prisma migrate deploy` → row-count/sentinel checks → three-platform regression (see checklist in chat).
- Known failures: none in sandbox.
- Next recommended task: Codex executes the Phase 3a migration checklist on the real DB, merges branch to `main` (AI_HANDOFF §12 conflict: branch version wins). After that: Phase 3b (metadata checker + availability writers + StatusBadge).
- Summary: Phase 3a — additive-only archive schema migration with rehearsed SQL, lastSeenAt now recorded on every refresh hit.
