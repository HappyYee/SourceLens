...
- Current status: UX batch U1 + U1.5 locally verified with downgraded non-browser checks and fast-forward merged into `main`; awaiting user-confirmed push. U2 (per-item read system) and U3 (settings overhaul + sidebar drag) approved and queued.
...
- Tests run: `npx prisma generate`; `npm test` 193/193; `npm run build` passed (only the known Google Fonts optimization warning). Local downgraded verification passed: `/api/refresh` with `scope=week` returned HTTP 200 (`bindings=10`, `updated=44`); two `force=true` runs returned HTTP 200 and the second was faster (`215.9s` → `170.1s`) with final failed count 0; `/` and `/?sort=recent` returned HTTP 200 and home HTML contains 「检查更新」/「上次刷新」/「最近更新」; during an X latest refresh the home page responded immediately (`200` in `0.14s`); X/Bilibili/YouTube latest returned failedCount=0. Visual-only UI items were verified by the user on the real machine (4/4), including the clickable 「最近更新」 sort toggle.
- Known failures: none. Note: Browser plugin policy blocked Codex from opening localhost for visual inspection in this session, so Codex used API/curl verification and relied on the user's visual confirmation for UI-only checks.
- Next recommended task: Push `main` after user confirmation; then Claude implements U2 (Item.readAt additive migration + read checkboxes + auto-mark-on-click + unread badges + home unread timeline + sources panel collapse).
